...
export default async function getSteamInfo() {
  let result: any = {
    name: 'username',
  ...
  try {
    ...
    result.recent_played_game =
      res.data.response.total_count > 0 ? res.data.response.games[0].name : '';
    res = await axios.get(
    ...
    const date = new Date(
      res.data.response.players[0].lastlogoff * 1000
    ).toLocaleString();
